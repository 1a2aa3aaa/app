<template>
  ...
  infinite-scroll-disabled="loading"
   infinite-scroll-distance="0"><!--//离页尾还有多少像素时触发 -->
  <li v-for="(item,i) in rows" :key="i">
     <img @click="del(ppid[i],i)" v-if="uid[i]==myid" class="posi-sc" width="18px;" 
     :src="`${$store.state.pathurl}sc.png`" alt="">
      <div id="list">
          ...
          <div class="bottom">  
             <a href="javascript:;"><img :src="`${$store.state.pathurl}zhuanfa1.png`" alt=""></a> 
             <a href="javascript:;" @click="scan(ppid[i],i)">
                 <img :src="`${$store.state.pathurl}${isc[i]}`" alt="">
              </a>  
             <router-link :to="`detil?pid=${ppid[i]}`"><img :src="`${$store.state.pathurl}xinxi1.png`" alt=""></router-link>
             <!-- 判断是否点过赞 -->
              <a href="javascript:;" @click="dzan(ppid[i],i)">
                  <!-- 函数传参,传该贴id,下标, -->
                  <img :src="`${$store.state.pathurl}${iszan[i]}`" alt="">
              <span :class="isactive[i]">
                {{pzan[i]}}
                </span></a>    
          </div>
      </div>
...
export default {
    ...
    methods: {
        del(pid,i){//删帖
         ...
         this.$messagebox.confirm("是否删除该贴").then(action=>{
              var url="detail/del";
               ...
               this.axios.delete(url,{params:obj}).then(res=>{
        ...
        loadList (page,sid) {//下拉执行的函数，参数为页码
            //发送ajax请求
            var url=this.url;
            ...
            this.axios.get(url,{params:obj}).then(res => {
                //得到点赞表帖子的id
                //console.log(1);
              if(this.page==1){  //因为第一次就能得到赞表的所有值,不需要二次
         ...
         isagein(rows){//判断是否还有数据
              if(rows.length==0){//当没有数据了，开关false
         ...
         list(){//判断点赞与否
         //不登录返回的zpid就是0
                 if(this.zpid.length>0){//如果登陆了
                 //第一遍从0开始,往后加5
                     for(var i=(this.page-1)*5;i<this.ppid.length;i++){//遍历判断两个数组里是否有g等的
                     this.iszan[i]="dianzan1.png";//是否点赞默认否
                     this.isactive[i]="noactive";
                     //获取zpid里我点赞了几个
                     for(var j=0;j<this.zpid.length;j++){
                         //遍历找到点赞的每个帖子下标
                         if(this.ppid[i]==this.zpid[j]){//有就提出来
            ...
            //不登录返回的zpid就是0
                 ...
                 //第一遍从0开始,往后加5
                     ...
                     for(var j=0;j<this.cpid.length;j++){
                         //遍历找到点赞的每个帖子下标
                         if(this.ppid[i]==this.cpid[j]){//有就提出来
   ...
   dzan(ppid,i){
     //获得该贴的下标,和帖子id
     if(this.iszan[i]=="dianzan.png"){//点过赞
         var is=1;
     }else{//没点过
         var is=0;
     }
    ...
    // 收藏功能
     ...
     if(this.isc[i]=="shoucang.png"){//收藏过
         var is=1;
     }else{//没收藏过
         var is=0;
     }
